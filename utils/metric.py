...
class Loss(nn.Module):
    ...
    def diversity_loss2(self,features):
        ...
        # 计算损失
        diversity_loss = (torch.sum(loss_tentor) - 1 * batch_size * num_parts) / (num_parts * (num_parts - 1))
    ...
    def forward(self, image_embeddings, text_embeddings,img_f,text_f,labels,lambda_diversity):
        ...
        if self.CMPM:
            ...
            for i in range(len(image_embeddings)):
                cmpm_loss1, pos_avg_sim, neg_avg_sim = self.compute_cmpm_loss(image_embeddings[i], text_embeddings[i], labels)
                # if i==0:
                cmpm_loss+=cmpm_loss1
                # else:
                #     cmpm_loss += cmpm_loss1*1.0/(len(image_embeddings)-1)

        if self.CMPC:
            cmpc_loss=0
            for i in range(len(image_embeddings)):
                cmpc_loss1, image_precision, text_precision = self.compute_cmpc_loss(image_embeddings[i], text_embeddings[i], labels)
                # if i==0:
        ...
        loss+=lambda_diversity*(self.diversity_loss(img_f[:,1:])+self.diversity_loss(text_f[:,1:]))
