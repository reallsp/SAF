...
def train(epoch, train_loader, network, optimizer, compute_loss, args):
    ...
    for step, (images, captions, labels) in enumerate(train_loader):
        ...
        if (args.constraints_images or args.constraints_text) and step == len(
            train_loader
        ) - 1:
            ...
            print(
                "epoch:{}, step:{}, con_images:{:.3f}, con_text:{:.3f}".format(
                    epoch, step, con_images.item(), con_text.item()
                )
            )
    ...
    for epoch in range(1, args.num_epoches + 1 - args.start_epoch):
        ...
        logging.info(
            "Epoch:  [{}|{}], train_time: {:.3f}, train_loss: {:.3f}".format(
                args.start_epoch + epoch, args.num_epoches, train_time, train_loss
            )
        )
        logging.info(
            "image_precision: {:.3f}, text_precision: {:.3f}".format(
